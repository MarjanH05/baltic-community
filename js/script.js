...
// DOM Elements - These are declared globally. Functions that use these should check they exist before using it otherwise **IT WILL BREAK** (fails silently breaking the script).
...
if (signupBtn && loginPage && signupPage) {
...
if (backToLoginBtn && loginPage && signupPage) {
...
function renderNotifications() {
    ...
    if (!currentUser || !currentUser.id) {
        notificationsList.innerHTML = '<div class="notification-item">Please log in to see notifications.</div>';
        return;
    }
...
function handlePostSubmit() {
    ...
    if (content && content.trim() !== '') {
        ...
        // This is unnecesary until we add the spinner back. Remove comment when we have. 
        //setTimeout(() => { 
        //    postInput.value = '';
        //    autoResizeTextarea();
        //}, 250);
...
function autoResizeTextarea() {
    if (!postInput) return;
    postInput.style.height = '36px'; // Reset height 
...
// Initialize postInput specific styles and listeners
if (postInput) {
...
function updateDropdownProfile() {
    ...
    dropdownUsername.textContent = currentUser.fullName || currentUser.name;
...
if (logoutLink) {
    logoutLink.addEventListener('click', function () {
        ...
        // Or, if on index.html, ensure the login form is visible.
        if (window.location.pathname.endsWith('index.html') || window.location.pathname === '/') {
            if (loginPage) loginPage.classList.remove('hidden');
        }
        else {
            // If on another other pages, redirect to index or login page
...
function loginUser(user, fromSession = false) {
    ...
    // Update UI
    if (userNameSpan) {
        userNameSpan.textContent = (user.fullName || user.name)
            .split(' ')
...
function updateCurrentUserAvatar() {
    if (currentUser && profileIcon) { // Ensure currentUser and profileIcon exist
        if (currentUser.avatar) {
            profileIcon.innerHTML = `<img src="${currentUser.avatar}" alt="${currentUser.name}" class="avatar-image">`;
        } else {
            profileIcon.textContent = (currentUser.fullName || currentUser.name).charAt(0);
        }
    }

    // Post input avatar - only if postUserAvatar element exists (typically on index.html)
    if (postUserAvatar && currentUser) {
        if (currentUser.avatar) {
            postUserAvatar.innerHTML = `<img src="${currentUser.avatar}" alt="${currentUser.name}" class="avatar-image">`;
        } else {
            postUserAvatar.textContent = (currentUser.fullName || currentUser.name).charAt(0);
        }
    }
}
...
function renderPosts() {
    if (!postsContainer) return; // Guard from script.js
    ...
    posts.forEach((post, index) => {
        ...
        const truncatedContent = showReadMore ? post.content.slice(0, maxLength) + '...' : post.content;
            ...
            <div class="post-header">
                <div class="post-author">
                    <div class="author-avatar">
                        ${post.avatar ? `<img src="${post.avatar}" alt="${post.authorName}" class="avatar-image">` : post.authorName.charAt(0)}
                    </div>
        ...
        setupPostOptionsMenu(); // Call after each post to ensure menu is set up
    });
...
function setupPostOptionsMenu() {
    if (!postsContainer) return; // Guard from script.js
    const postOptions = postsContainer.querySelectorAll('.post-options');
...
function editPost(index) {
    if (!postsContainer || !posts[index]) return; // Guard from script.js
    const post = posts[index];
...
async function deletePost(index) {
    if (!posts[index]) return; // Guard from script.js
    ...
    users.forEach(user => {
        ...
        userElement.innerHTML = `
            <div class="user-avatar-small">
                ${user.avatar ? `<img src="${user.avatar}" alt="${user.name}" class="avatar-image">` : (user.fullName || user.name).charAt(0)}
            </div>
        ...
        eventElement.innerHTML = `
            <div class="event-date" style="
                ...
                <div id="top" style="margin: -1px -1px 0 -1px;
                    ...
                    color: white;">${event.month.substring(0, 3)}</div>
